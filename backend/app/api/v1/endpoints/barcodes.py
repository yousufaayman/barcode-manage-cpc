...
def print_barcode_zebra(
    ...
    try:
        ...
        def clean_ascii_text(text: str) -> str:
            if not text:
                return ""
            # Strip non-ASCII for predictable ZPL on Latin fields
            cleaned = str(text).replace('\n', ' ').replace('\r', ' ').replace('\t', ' ')
            return cleaned.encode('ascii', 'ignore').decode('ascii').strip()

        def zpl_safe_utf8(text: str) -> str:
            """Keep UTF-8 (Arabic allowed) but remove ZPL control chars and newlines."""
            if not text:
                return ""
            return (
                str(text)
        ...
        def shape_arabic_for_zpl(text: str) -> str:
            """
            Zebra doesn't reliably Arabic-shape/bidi dynamic UTF-8 in ^FD.
            Pre-shape (presentation forms) + apply bidi reordering similar to python-bidi get_display().
            """
            safe = zpl_safe_utf8(text)
            if not safe:
                return ""
            # Arabic blocks + Arabic Presentation Forms ranges
            import re

            if not re.search(r"[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]", safe):
                return safe
            try:
                import arabic_reshaper
                from bidi.algorithm import get_display

                reshaped = arabic_reshaper.reshape(safe)
                return get_display(reshaped)
            except Exception:
                # Fallback: at least keep UTF-8 text intact
                return safe

        client = clean_ascii_text(brand)
        model = clean_ascii_text(model_name)
        color = clean_ascii_text(color_name)
        qty = str(int(quantity) if quantity is not None else 0)
        size = clean_ascii_text(size_value)
        # Label says "Serial:" on sticker; value is batch.layers (legacy wording).
        layers_s = str(int(layers) if layers is not None else 1)
        po = clean_ascii_text(job_order_number)
        ...
        if is_rework:
            # Rework label: explicit layout, UTF-8 Arabic via ^CI28.
            # Variable fields may include Arabic; keep UTF-8 and pre-shape.
            phase_s = shape_arabic_for_zpl(phase_name or "")
            stage_s = shape_arabic_for_zpl(stage_name or "")
            color_utf8 = shape_arabic_for_zpl(color_name or "")
            size_utf8 = shape_arabic_for_zpl(size_value or "")
            barcode_utf8 = zpl_safe_utf8(barcode_string or "")
            parts = [
                "^XA",
                "^CI28",
                "^PA0,1,1,1",
            ...
            if arabic_field:
                parts.append(arabic_field)
            else:
                parts.append("^FO580,340^A@N,50,50,E:SWISS271.TTF^FD\ufe94\ufef4\ufee7\ufe8e\ufe9b \ufe94\ufe9f\ufead\ufea9^FS")
            parts.extend(["^PQ1", "^XZ"])
            zpl_code = "\r\n".join(parts)
        else:
            parts = [
...
async def print_barcodes(
    ...
    try:
        ...
        for barcode in request.barcodes:
            ...
            # If this is a rework batch print request, mark printed on success.
            if bool(barcode.get("is_rework", False)) and barcode.get("rework_batch_id") is not None:
                try:
                    ...
                    # Printing succeeded; do not fail the request due to bookkeeping.
                    db.rollback()
