#!/usr/bin/env python3
"""
Windows Service Runner for Barcode Management Backend - Instance 1
This script runs the FastAPI application as a Windows service on port 5000
Optimized for production deployment with dual network support
"""
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - [INSTANCE_1] - %(message)s',
    handlers=[
        logging.FileHandler('C:/barcode-app/backend/logs/app_instance_1.log'),
...
async def lifespan(app):
    ...
    # Startup
    logger.info("Starting Barcode Management Backend - Instance 1...")
    try:
        # Add any startup tasks here (database connections, etc.)
        logger.info("Backend Instance 1 startup completed successfully")
        yield
        ...
        # Shutdown
        logger.info("Shutting down Barcode Management Backend - Instance 1...")
...
def get_production_config():
    """Get production-optimized configuration for Instance 1"""
    # Instance 1 specific configuration
    host = "0.0.0.0"  # Listen on all interfaces for dual network support
    port = 5000  # Instance 1 port
    
    # Production settings - disable reload, enable workers
    workers = int(os.getenv("WORKERS", "2"))  # Reduced workers per instance
    ...
    limit_concurrency = int(os.getenv("LIMIT_CONCURRENCY", "500"))  # Reduced per instance
    limit_max_requests = int(os.getenv("LIMIT_MAX_REQUESTS", "500"))
...
def main():
    ...
    try:
        ...
        logger.info(f"Starting FastAPI server - Instance 1 on {config['host']}:{config['port']}")
        logger.info(f"Workers: {config['workers']}")
        logger.info(f"Log level: {config['log_level']}")
        logger.info(f"Environment: Production - Dual Network Support")
