#!/usr/bin/env python3
"""
Windows Service Runner for Barcode Management Backend - Instance 2
This script runs the FastAPI application as a Windows service on port 5001
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - [INSTANCE_2] - %(message)s',
    handlers=[
        logging.FileHandler('C:/barcode-app/backend/logs/app_instance_2.log'),
...
async def lifespan(app):
    ...
    # Startup
    logger.info("Starting Barcode Management Backend - Instance 2...")
    try:
        # Add any startup tasks here (database connections, etc.)
        logger.info("Backend Instance 2 startup completed successfully")
        yield
        ...
        # Shutdown
        logger.info("Shutting down Barcode Management Backend - Instance 2...")
...
def get_production_config():
    """Get production-optimized configuration for Instance 2"""
    # Instance 2 specific configuration
    host = "0.0.0.0"  # Listen on all interfaces for dual network support
    port = 5001  # Instance 2 port
...
def main():
    ...
    try:
        ...
        logger.info(f"Starting FastAPI server - Instance 2 on {config['host']}:{config['port']}")
