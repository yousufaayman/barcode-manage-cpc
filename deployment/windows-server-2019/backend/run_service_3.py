#!/usr/bin/env python3
"""
Windows Service Runner for Barcode Management Backend - Instance 3
This script runs the FastAPI application as a Windows service on port 5002
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - [INSTANCE_3] - %(message)s',
    handlers=[
        logging.FileHandler('C:/barcode-app/backend/logs/app_instance_3.log'),
...
async def lifespan(app):
    ...
    # Startup
    logger.info("Starting Barcode Management Backend - Instance 3...")
    try:
        # Add any startup tasks here (database connections, etc.)
        logger.info("Backend Instance 3 startup completed successfully")
        yield
        ...
        # Shutdown
        logger.info("Shutting down Barcode Management Backend - Instance 3...")
...
def get_production_config():
    """Get production-optimized configuration for Instance 3"""
    # Instance 3 specific configuration
    host = "0.0.0.0"  # Listen on all interfaces for dual network support
    port = 5002  # Instance 3 port
...
def main():
    ...
    try:
        ...
        logger.info(f"Starting FastAPI server - Instance 3 on {config['host']}:{config['port']}")
